...
bool tesoro_dfs(vector<vector<char>> &T, int i, int j)
{
	...
	if (T[i][j] == 't')
		return true; // si estoy en 't' retorno cierto
	if (T[i][j] == 'X')
		return false; // si estoy en 'X' retorno falso
	...
	for (auto &d : direcciones)
	{
		...
		if (limitado(T, adj_i, adj_j) and tesoro_dfs(T, adj_i, adj_j))
			return true;
	}
...
int main()
{
	...
	for (int i = 0; i < n; ++i)
	{ // Itero sobre filas con variable i
		for (int j = 0; j < m; ++j)
		{ // Itero sobre columnas con variable j
			cin >> T[i][j];
		}
	}
